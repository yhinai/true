...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("benchmark-configs/baseline.yaml"),
        help="Path to baseline replay-smoke config.",
    )
...
def main() -> int:
    ...
    results = [run_baseline(load_task(task_path), app_config) for task_path in task_paths]
    summary = summarize_results(results)
    output = {
        "suite": "baseline-smoke",
    ...
    output_path = reports_root / "baseline-smoke" / "suite-summary.json"
